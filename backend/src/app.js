...
export function createApp() {
  ...
  app.use(express.urlencoded({ extended: false, limit: '50kb' })); // Africa's Talking callbacks
  if (!env.isTest) {
    // Never write query-string secrets (e.g. the Africa's Talking callback ?secret=) to access logs.
    morgan.token('url', (req) => (req.originalUrl || req.url).replace(/([?&](?:key|token|apiKey|secret)=)[^&]*/gi, '$1[REDACTED]'));
